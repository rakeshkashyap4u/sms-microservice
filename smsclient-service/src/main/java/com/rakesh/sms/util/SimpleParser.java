 ...
 *
 *          $Log: SimpleParser.java,v $ Revision 1.0 2 Agrim *** empty log
 *          message ***
 *
 * 
 */
...
public class SimpleParser {
	private static String[] asLogicalOperators = { "ex", "EX", "!ex", "!EX", "==", "=i", "!==", "!=i", ">>", "gt", "GT",
			">=", "ge", "GE", "<<", "lt", "LT", "<=", "le", "LE", "cc", "!cc", "CC", "!CC", "sw", "!sw", "SW", "!SW",
			"ew", "!ew", "EW", "!EW", "in", "!in", "IN", "!IN", "mod" };
	...
	private Vector<String> vTokens = null;
	...
	public SimpleParser(String source, boolean isStatic, boolean autoQuote) throws Exception {
		...
		StringTokenizer stText = new StringTokenizer(source, "$", true); // use '$' as separator, and return it as a
																			// token
		if (isStatic || stText.countTokens() < 2) {
				...
				do {
					...
					if (pos2 == -1)
						...
						source = source.substring(0, pos1 + 4)
								+ getQuotedPrintableText(source.substring(pos1 + 4, pos2)) + source.substring(pos2);
			...
			while (stText.hasMoreTokens()) {
				...
				if (szToken.charAt(0) == '$') { // this is a separator
					...
					case 2: // the last token was a tag content; add an empty element and the tag content to
							// the vector
	...
	public String parse(Properties p) {
		...
		String szModifier;
		StringBuffer sbText;
		Stack<Boolean> stackShowComment = new Stack<Boolean>();
		int iCommentFlag = 0;
			...
			for (int iCount = 0; iCount < vTokens.size(); iCount++) {
				try {
					...
					if (szToken == null) { // next token is a tag content
						szToken = (String) vTokens.elementAt(++iCount);
						// translate tag
						if (szToken.charAt(0) == '(') { // there is a tag modifier
							int iPointer = szToken.indexOf(')');
							szModifier = szToken.substring(1, iPointer);
							if (szModifier.equalsIgnoreCase("!comment")) { // look for the following name; if it is
																			// found in the properties, set the comment
																			// flag to true
							...
							} else if (szModifier.equalsIgnoreCase("!show")) { // look for the following name; if it is
																				// found in the properties, set the
				...
				do {
					...
					if (pos2 == -1)
						szSource = szSource.substring(0, pos1 + 4)
								+ getQuotedPrintableText(szSource.substring(pos1 + 4));
					else if (pos2 > pos1 + 4)
						szSource = szSource.substring(0, pos1 + 4)
								+ getQuotedPrintableText(szSource.substring(pos1 + 4, pos2)) + szSource.substring(pos2);
	...
	private boolean ConditionMatched(String szCondition, Properties Pr) throws Exception {
		...
		for (int i = 0; i < asLogicalOperators.length; i++) {
			...
			if (iLogicalOperatorPos != -1) {
				...
				if (!bUnaryOperator) {
					if (szCondition.length() <= iLogicalOperatorPos + asLogicalOperators[iLogicalOperatorFound].length()
							+ 2) {
						throw new Exception("Error in SimpleParser.MatchCondition: Binary Operator has not parameter");
					}
					szValue = szCondition
							.substring(iLogicalOperatorPos + asLogicalOperators[iLogicalOperatorFound].length() + 2)
							.trim();
	...
	public static String parseQuotedPrintableText(String qsource) {
		...
		while ((pos = qsource.indexOf("=", pos + 1)) != -1)
			qsource = qsource.substring(0, pos) + (char) Integer.parseInt(qsource.substring(pos + 1, pos + 3), 16)
					+ qsource.substring(pos + 3);
		return qsource;
	}
}// SimpleParser
